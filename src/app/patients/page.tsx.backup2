...
export default function PatientsPage() {
  ...
  function rdvDuPatient(patientId: number): RendezVous | undefined {
    return rdvs
      .filter(r => r.patientId === patientId && r.statut !== 'annule')
      .sort((a, b) =>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-outline-variant bg-surface-container-low/50">
                ...
                <th className="table-header">Rendez-vous</th>
                    ...
                    <td className="table-cell text-xs">
                      {rdv ? (
                        <span className="text-on-surface-variant">
                          {rdv.date} · {(rdv.heureDebut || '').slice(0, 5)}
                          {rdv.statut === 'en_cours' && <span className="ml-2 text-emerald-600 font-semibold">en cours</span>}
                        </span>
                      ) : (
